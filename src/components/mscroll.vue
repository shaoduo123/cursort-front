  ...
  <div class="my-scroll" :class="[scrollState?'prohibit':'allow']" ref="myScroll" @scroll.passive="onScroll($event)"  @touchmove="onScroll($event)"  >
    ...
    <div class="scroll-list">
      ...
      <div class="scroll-bottom">
        <div v-if="state==1">
          <i><img src="../assets/load.gif" style="width: 20px;height: 20px"></i>
          <p style="font-size: 12px;margin: 10px  ;color: #d2d2d2" >加载中</p>
        </div>
<!--        <div v-if="state==2">下滑加载</div>-->
        <div v-if="state==3" style="font-size: 12px;margin: 10px; color: #d2d2d2">-没有数据了-</div>
      </div>
    </div>
  </div>
</template>
